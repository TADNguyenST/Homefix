...
const fileFilter = (req, file, cb) => {
  ...
  return cb(new Error('Chi chap nhan file anh (JPEG, PNG, WebP, GIF)'), false);
};
...
router.post('/', authMiddleware, roleMiddleware(['ADMIN', 'CUSTOMER', 'TECHNICIAN']), upload.single('image'), (req, res) => {
  if (!req.file) {
    return res.status(400).json({ success: false, message: 'Khong co file nao duoc tai len' });
  }
...
router.delete('/', authMiddleware, roleMiddleware(['ADMIN', 'CUSTOMER', 'TECHNICIAN']), async (req, res) => {
  ...
  const [bookingReference, serviceReference, blogReference] = await Promise.all([
    ...
    prisma.blog.findFirst({ where: { image_urls: { has: imageUrl } }, select: { id: true } }),
  ]);
