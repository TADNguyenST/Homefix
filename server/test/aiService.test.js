...
test('phân loại lỗi mạng có mã hệ thống thành lỗi kết nối Gemini', () => {
  const source = new Error('socket disconnected');
  source.code = 'ECONNRESET';
  const error = _test.classifyGeminiError(source);
  assert.equal(error.code, 'GEMINI_CONNECTION_FAILED');
});

test('chuẩn hóa điểm gợi ý thợ và loại bỏ ID không thuộc danh sách ứng viên', () => {
  const validIds = new Set([10]);
  const accepted = _test.normalizeRecommendation({
    technician_id: '10',
    ai_score: 140,
    ai_reason: '  Có kỹ năng và lịch phù hợp.  ',
  }, validIds);
  const rejected = _test.normalizeRecommendation({
    technician_id: 99,
    ai_score: 90,
    ai_reason: 'Không thuộc danh sách ứng viên',
  }, validIds);
